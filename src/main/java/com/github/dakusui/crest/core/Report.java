...
public interface Report {
  ...
  static <T> Report perform(T value, Matcher<T> matcher) {
    return perform(value, matcher, Session.create());
  }
  ...
  interface Session<T> {
    ...
    class Impl<T> implements Session<T> {
      ...
      void describeExpectationTo(Writer writer, Matcher.Leaf<T> matcher) {
      ...
      @SuppressWarnings("unchecked")
      private void explainFunction(T value, Function<T, ?> func, String variableName, Writer writer) {
      ...
      @SuppressWarnings("unchecked")
      private <I, O> void explainChainedFunction(I value, Call.ChainedFunction<I, O> chained, String variableName, Writer writer) {
        writer.enter();
        try {
