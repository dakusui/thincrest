|   | # | 機能 	| Hamcrest 	| AssertJ |
|---|---| ----- | --------- | ------- |
|   | 1 |チェック対象を指定する | assertThat |assertThat|
...
|   | 16|大文字・小文字・ブランク文字を無視して比較する 	|is + equalToIgnoringWhiteSpace 	|isEqualToIgnoringWhitespace|
...
|   | 20|数値であることをチェックする 	|- 	|containsOnlyDigits|
|   | 21|行数をチェックする 	|- 	|hasLineCount|
|   | 22|指定した順序で文字列が現れることをチェックする 	|is + stringContainsInOrder 	|containsSequence|
|   | 23|指定した範囲の値であることをチェックする 	|is + closeTo 	|isBetween|
|   | 24|指定した値より大きいことをチェックする 	|is + greaterThan 	|isGreaterThan|
|   | 25|指定した値以上であることをチェックする 	|is + greaterThanOrEqualTo 	|isGreaterThanOrEqualTo|
|   | 26|指定した値より小さいことをチェックする 	|is + lessThan 	|isLessThan|
|   | 27|指定した値以下であることをチェックする 	|is + lessThanOrEqualTo 	|isLessThanOrEqualTo|
|   | 28|全ての要素が等しいことをチェックする 	|is + contains 	|contains|
|   | 29|指定した要素を持つことをチェックする 	|hasItem 	|containsOnly|
|   | 30|指定した要素を全て含むことをチェックする 	|hasItems 	|containsOnly|
|   | 31|指定したサイズであることをチェックする 	|hasSize 	|hasSize|
|   | 32|空であることをチェックする 	|is + empty 	|isEmpty|
|   | 33|空の Iterator であることをチェックする 	|is + emptyIterable 	|isEmpty|
...
|   | 39|定した要素と配列の要素が全て等しいことをチェックする 	|is + arrayContaining 	|containsExactly|
|   | 40|順序を無視して、指定した要素が全て配列に含まれることをチェックする 	|is + arrayContainingInAnyOrder 	|contains|
|   | 41|配列のサイズをチェックする 	|is + arrayWithSize 	|hasSize|
|   | 42|配列が空であることをチェックする 	|is + emptyArray 	|isEmpty|
|   | 43|配列の中に指定した値が存在することをチェックする 	|hasItemInArray 	|contains|
